...
function VerdictContent() {
  ...
  const [realityMission, setRealityMission] = useState("");
  const [error, setError] = useState(false);

  // 로컬 폴백 판결문 및 미션 생성 함수
  const generateFallbackVerdict = useCallback((
    nick: string,
    task: string,
    days: number,
    km: number,
...
피고인이 자진 자수한 점을 고려하여 형을 감경한다.`;
    ...
    return {
      text,
      sentence: sent,
      realityMission: fallbackMission
    };
  }, []);

  // 판결문 요청 생성 함수
  const generateVerdict = useCallback(async () => {
    if (!currentEscape) return;
    ...
    try {
      ...
      // ⭐ 핵심 수정: 분리된 판결문 API와 현실 복귀 명령 API를 병렬로 동시 호출합니다.
      const [verdictRes, missionRes] = await Promise.all([
        fetch("/api/verdict", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(requestBody),
        }),
        fetch("/api/reality-mission", { // 분리된 미션 라우트 주소에 맞게 확인해 주세요
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(requestBody),
        })
      ...
      if (!verdictRes.ok || !missionRes.ok) {
        throw new Error("API 멀티 엔드포인트 호출 실패");
      }
      ...
      try {
        const saved = await createVerdict({
        ...
        setCurrentVerdict({
          ...(saved as Verdict),
          realityMission: parsedMission 
        });
      } catch (e) {
        console.error("판결 저장 실패:", e);
      }
    } catch (err) {
      ...
      setRealityMission(fallback.realityMission);
    } finally {
      setLoading(false);
    }
  }, [nickname, currentEscape, reason, generateFallbackVerdict, setCurrentVerdict]);
  ...
  useEffect(() => {
    ...
    generateVerdict();
  }, [nickname, currentEscape, router, generateVerdict]);
  ...
  return (
    <div style={{ background: "#0A0A0A", minHeight: "100vh" }}>
      <nav style={{ background: "#FFD600", borderBottom: "3px solid #0A0A0A", padding: "10px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontFamily: "'Black Han Sans',sans-serif", fontSize: "20px", color: "#0A0A0A" }}>
          ⚖️ 판결문
        </span>
        <span style={{ fontSize: "12px", fontWeight: 700, color: "#0A0A0A", opacity: 0.7 }}>
          {character} {nickname}
        </span>
      </nav>
      ...
      <div style={{ maxWidth: "480px", margin: "0 auto", padding: "20px 16px" }}>
        ...
        >
          ...
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: verdictText.replace(/\n/g, "<br/>"),
            }}
          />

          <div style={{ textAlign: "center", background: "#fff8e0", border: "2px solid #ccc", padding: "14px", borderRadius: "2px", marginTop: "16px" }}>
        ...
        <button
          ...
          style={{ width: "100%", background: "#E8162E", color: "#fff", border: "none", padding: "14px", fontFamily: "'Black Han Sans', sans-serif", fontSize: "18px", letterSpacing: "2px", cursor: "pointer", borderRadius: "2px", marginTop: "10px" }}
        >
          🏛️ 수감증 받기
        </button>
...
export default function VerdictPage() {
  return (
    <Suspense fallback={<div style={{ background: "#0A0A0A", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}><div style={{ color: "#FFD600", fontFamily: "'Black Han Sans',sans-serif" }}>로딩 중...</div></div>}>
      <VerdictContent />
    </Suspense>
  );
}